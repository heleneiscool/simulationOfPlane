...
public class TarmacShuttle {
    ...
    public List<Event> run(Set<Passenger> passengers) {
        ...
        Queue<Passenger> plane = new PriorityQueue<Passenger>(passengers);
        List<Queue<Passenger>> shuttleQueues = new ArrayList< Queue<Passenger>>();
        for(Shuttle shuttle : shuttles){
            shuttleQueues.add(new PriorityQueue<Passenger>());
        }
        // loop
        while(true) {
            ...
            for(Queue<Passenger> queue : shuttleQueues){
                if(queue.size()<SHUTTLE_QUEUE_MAX & plane.size()>0){
                    Passenger p  = plane.poll();
                    queue.add(p);
                    record.add(new Event("deplane", p, shuttleQueues.indexOf(queue))); 
                    break;
                }
            }
            for(Shuttle shuttle: shuttles){
                if(!shuttle.isWaiting()){
                    shuttle.advanceTrip();
                    continue;
                }
                if(!shuttle.isFull() & !shuttleQueues.get(shuttle.getNum()).isEmpty()){
                    Passenger p = shuttleQueues.get(shuttle.getNum()).poll();
                    shuttle.addPassenger(p);
                    record.add(new Event("onShuttle", p, shuttle.getNum()));
                    continue;
                }
                if(!shuttle.isEmpty()){
                shuttle.startTrip();
                record.add(new Event("toTerminal", shuttle.getNum())); 
            }
            }
            ...
            for(Shuttle shuttle : shuttles){
                if(!shuttleQueues.get(shuttle.getNum()).isEmpty() | !shuttle.isEmpty()){finished = false;}
            }
            if(finished){break;}
